...
uniprot = pd.read_csv("/Users/rieke/Desktop/typhle_dedup.csv")
liste = []
liste_genes = []
...
for name in id_list:
    ...
    file = open("/Users/rieke/Desktop/tyhple_dedup_id.txt", 'r')
    for line in file:
    ...
    if(len(a) !=0):
        for i in a:
            for b in range(1, len(a.get(i))):
                ...
                del temp
                gc.collect()
                d3.to_csv("/Users/rieke/Desktop/typhle_check" + name.replace(".txt", "") + ".csv")
    del d3
    gc.collect()
    a.clear()
    liste.clear()
