...
query_ensembl =  "SELECT gene.stable_id, dna.sequence, gene.seq_region_start, gene.seq_region_end, gene.description FROM dna INNER JOIN " \
...
for x in data_extraction.id:
    query_own = "INSERT INTO danio_rerio (id,seq,descr) VALUES (\"" + str(x) + "\",\" " + str(data_extraction.seqcut[i]) + "\", \"" + str(data_extraction.description[i])+ "\"); "
    i = i+1
